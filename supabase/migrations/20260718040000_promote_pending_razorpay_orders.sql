-- payment.captured promotes the pending row made at Razorpay order creation.
-- Replacing the function preserves idempotency for duplicate webhook deliveries.
...
BEGIN
  ...
  DO UPDATE SET
    ...
    items = CASE WHEN orders.status = 'pending' THEN EXCLUDED.items ELSE orders.items END,
    subtotal = CASE WHEN orders.status = 'pending' THEN EXCLUDED.subtotal ELSE orders.subtotal END,
    total = CASE WHEN orders.status = 'pending' THEN EXCLUDED.total ELSE orders.total END,
    shipping_address = CASE WHEN orders.status = 'pending' THEN EXCLUDED.shipping_address ELSE orders.shipping_address END,
    payment_method = 'razorpay',
    payment_id = COALESCE(orders.payment_id, EXCLUDED.payment_id)
  ...
  SELECT
    v_order_id, p_customer_id, source.ordinality - 1,
